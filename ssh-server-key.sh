...
###############################################################################
echo "[2] ВЪВЕЖДАНЕ НА ПСЕВДОНИМ ЗА SSH КЛЮЧОВЕТЕ..."
echo "-------------------------------------------------------------------------"

# Допустими символи: букви, цифри, тире и долна черта. Дължина: 3 до 15 знака.
while true; do
  echo "Въвеждане на псевдоним за SSH ключовете"
  echo "Този псевдоним ще се използва за името на ключовите файлове"
  read -rp "(букви, цифри, '-', '_', 3-20 символа): " server_alias
...
echo "y - стартира скрипта отначало"
echo "n - изтрива този скрипт и приключва."
echo ""
...
while true; do
  ...
  case "$choice" in
    ...
    [Nn])
      ...
      echo "🧹 Самоунищожаване на скрипта..."
      SCRIPT_PATH=$(realpath "$0")
      rm -f "$SCRIPT_PATH"
      echo "✅ Скриптът беше премахнат."
      exit 0
      ;;
    *)
