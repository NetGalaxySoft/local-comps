...
# =====================================================================
# [МОДУЛ 7] ОБОБЩЕНИЕ И ФИНАЛЕН ОТЧЕТ
# =====================================================================
echo "[7] ОБОБЩЕНИЕ НА ИЗПЪЛНЕНИЕТО..."
echo "-----------------------------------------------------------"
echo ""

SCRIPT_PATH="$HOME/.local/bin/unicyrl.py"
AUTOSTART_PATH="$HOME/.config/autostart/unicyrl.desktop"

# 1. Потвърждение за местоположението на скрипта
if [[ -f "$SCRIPT_PATH" ]]; then
  echo "📌 Скриптът UniCyrl е инсталиран тук:"
  echo "    $SCRIPT_PATH"
else
  echo "⚠️ Скриптът UniCyrl не беше намерен в очакваната локация: $SCRIPT_PATH"
fi

echo ""

# 2. Потвърждение за автоматично стартиране
if [[ -f "$AUTOSTART_PATH" ]]; then
  echo "🖥️ Автоматично стартиране е конфигурирано чрез:"
  echo "    $AUTOSTART_PATH"
else
  echo "⚠️ Не е открит .desktop файл за автоматично стартиране."
fi

echo ""

# 3. Проверка за зависимостта
if python3 -c "import keyboard" 2>/dev/null; then
  echo "✅ Зависимостта 'keyboard' е налична."
else
  echo "❌ Липсва зависимостта 'keyboard' – проверете ръчно."
fi

echo ""

# 4. Инструкция за ръчно стартиране
echo "🧪 За ръчен тест, изпълнете в терминала:"
echo "    python3 $SCRIPT_PATH"

echo ""

# 5. Как да проверим дали скриптът вече работи
echo "ℹ️ Ако UniCyrl е активен, натискането на 'Ctrl+Shift+C' би трябвало да копира текущия ред,"
echo "    преобразуван в кирилица, в клипборда."
echo "    Можете да го поставите с Ctrl+V, за да проверите резултата."
